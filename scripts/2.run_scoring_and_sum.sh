...
run_plink_scoring () {
    ...
    num_cols=$( head -1 all_harmonized_weights/${trait}.all_harmonized_weights.ALL_SNPS.ALL_CHRS.txt | wc -w ) 
    ${obj_tool_dir}/parallel -j $num_at_a_time ${obj_tool_dir}/plink2 --threads $num_threads --memory $mem_per_score --bfile $obj_plink_file_prefix{} --score all_harmonized_weights/${trait}.all_harmonized_weights.ALL_SNPS.chr{}.txt cols=fid,scoresums list-variants no-mean-imputation header-read --score-col-nums 4-$num_cols --out scores/raw_scores/${trait}.ACAF.chr{} ::: {22..1} X
