...
combine_PRS = function(
	...
	for (score_file_i in 1:length(score_files_list)) {
		...
		dd=fread(score_file)
		dd%<>%select(IID, ends_with("_SUM"))
		print(nrow(dd))
		if(is.null(all_scores)){
			all_scores<-dd
			print("creating all_score")
			print(paste("all_score has", ncol(all_scores), "columns"))
		}
		else{
		all_scores%<>%left_join(dd, by="IID")
		print("adding to all_scores")
		print(paste("all_score has", ncol(all_scores), "columns"))
		}
		#dd = fread(score_file)
	...
	#print(head(colnames(all_scores)))
	...
	for (train_size in train_size_list) {
		...
		######INSERT RESID SCALE FUNCTION#####
resid_scale <- function(data, prs_name, pcs = paste0("PC", 1:10)) {
  print("Starting PC resid process")
  pcs <- pcs[pcs %in% names(data)]
  if (length(pcs) == 0) stop("No PC columns found in `data`.")
  
  # Handle multiple PRS names
  existing_prs <- prs_name[prs_name %in% names(data)]
  if (length(existing_prs) == 0) stop("No PRS columns found in `data`.")
  
  # If single PRS, return vector; if multiple PRS, return data frame
  if (length(existing_prs) == 1) {
    formula <- as.formula(paste(existing_prs, "~", paste(pcs, collapse = " + ")))
    fit <- lm(formula, data = data)
    pred <- predict(fit, newdata = data)
    resid <- data[[existing_prs]] - pred
    z_resid <- as.numeric(scale(resid))
    print("Using PC resid scores (single)")
    return(z_resid)
  } else {
    # Multiple PRS - return matrix/data.frame
    results <- sapply(existing_prs, function(prs) {
      formula <- as.formula(paste(prs, "~", paste(pcs, collapse = " + ")))
      fit <- lm(formula, data = data)
      pred <- predict(fit, newdata = data)
      resid <- data[[prs]] - pred
      z_resid <- as.numeric(scale(resid))
    ...
    print("Using PC resid scores (multiple)")
    return(results)
  }
}
		...
		print(head(colnames(train_df)))
		print(length(colnames(train_df)))
		print(head(score_names))
		print(length(score_names))
		print("line 213")		
		...
		## --- PC residualization & train-based scaling to create z_best ---
			pcs <- paste0("PC", 1:10)                                 
			pcs <- pcs[pcs %in% colnames(train_tmp)]             
			if (length(pcs) == 0) stop("No PC columns (PC1..PC10) found in training data")

			# Fit PRS ~ PCs on TRAIN
			pcmod <- lm(as.formula(paste0(bestPRS, " ~ ", paste(pcs, collapse = " + "))), data = train_tmp)

			# Residualize PRS on TRAIN and scale using TRAIN mean/sd
			resid_train <- train_tmp[[bestPRS]] - predict(pcmod, newdata = train_tmp)
			mu  <- mean(resid_train, na.rm = TRUE)
			sdv <- sd(resid_train,  na.rm = TRUE)
			if (sdv == 0 || is.na(sdv)) stop("Zero/NA SD after residualization on training data")

			train_tmp$z_best <- (resid_train - mu) / sdv

			# Make test_data1 now so we can add z_best for test too (needed for caret predict + glm)
			test_data1 <- test_data
			resid_test <- test_data1[[bestPRS]] - predict(pcmod, newdata = test_data1)
			test_data1$z_best <- (resid_test - mu) / sdv
		## --- end residualization block ---


			formula_null = as.formula(paste0("trait ~ ", paste0(covar_list, collapse="+")))

			formula_bestPRS <- as.formula(paste0("trait ~ z_best + ", paste0(covar_list, collapse = "+")))
			...
			for (pval_thres in pval_thres_list) {
				...
				if (length(topprs) == 0) {
					...
					if (!isbinary) {
						...
						x_train[,topprs] = resid_scale(x_train, prs_name=topprs)
						print(paste("Debugging: pc resid function applied:",head(x_train$topprs)))					
						y_train = as.vector(train_df$trait)
						train_data = data.frame(x_train,trait=y_train)
						...
						if (length(topprs) == 1) {
							...
							# print(ww)
							test_data1 = test_data
							test_data1[,topprs] = as.numeric(resid_scale(test_data1, prs_name = topprs))
				...
				if (length(topprs) == 0) {
					...
					if (!isbinary) {
						...
						if (length(topprs) == 1) {
							...
							test_data1 = test_data
							test_data1[,topprs] = resid_scale(test_data1, prs_name=topprs)
