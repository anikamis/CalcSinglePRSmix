...
eval_single_PRS = function(data_df, pheno = "trait", prs_name, covar_list, isbinary=F, liabilityR2=F, alpha=0.05, regression_output=F) {
  ...
  ###ADDING IN PC RESIDS
    data_use <- if ("data.table" %in% class(data_df)) data.table::copy(data_df) else data_df #take care of this whole datatable dataframe situation

    pcs <- paste0("PC", 1:10)
    pcmod <- lm(as.formula(paste0(prs_name, " ~ ", paste(pcs, collapse = " + "))), data = data_use)
    pred_vals <- predict(pcmod, newdata = data_use)
    data_use$z_prs <- as.numeric(scale(data_use[[prs_name]] - pred_vals))
  ###

	if (isbinary & !liabilityR2) {
		data_use$trait = as.numeric(data_use$trait)

		formula = as.formula(paste0(pheno, " ~ z_prs + ", paste0(covar_list, collapse="+")))
		model_full = glm(formula, data=data_use, family="binomial")
		...
		formula = as.formula(paste0("trait ~ z_prs + ", paste0(covar_list, collapse="+")))
		model_full = lm(formula, data=data_use)
